...
class ScratchPythonBuilder:
    ...
    def start_connection(self, start_block_id=None):
        ...
        if block.connections:
            # If already connected, ask if user wants to disconnect or add another
            response = messagebox.askyesnocancel(
                "Sequence Connection",
                f"This block already has {len(block.connections)} sequence connection(s).\n"
                f"Do you want to disconnect all existing connections first?"
            )
            
            if response is None:  # Cancel
                return
            elif response:  # Yes, disconnect first
    ...
    def start_end_connection(self, control_block_id=None):
        ...
        if block.end_connection:
            ...
            response = messagebox.askyesnocancel(
                "End Connection",
                f"This block already has an end connection.\n"
                f"Do you want to disconnect the existing end connection first?"
            )
    ...
    def start_continue_connection(self, block_id=None):
        ...
        if block_id is None:
            if self.selected_block_id:
                ...
                messagebox.showwarning("No Block Selected",
                                       "Please select a block to start a continue connection.")
                return
        ...
        if block.continue_connection:
            ...
            response = messagebox.askyesnocancel(
                "Continue Connection",
                f"This block already has a continue connection to '{self.blocks[block.continue_connection].text}'.\n"
                f"Do you want to disconnect the existing continue connection first?"
            )
            ...
            if response is None:  # Cancel
                ...
                # After disconnecting, continue with setting up new connection
        ...
        print(f"Starting continue connection from block: {block_id}")  # Debug
    ...
    def handle_continue_click(self, x, y):
        ...
        if clicked_block_id and clicked_block_id != self.start_connection_block:
            ...
            print(f"Continue connection established: {self.start_connection_block} -> {clicked_block_id}")
    ...
    def cancel_continue_connection(self, event=None):
        ...
        if hasattr(self, 'project_name_label'):
            self.project_name_label.config(text=self.project_name)
        self.root.unbind("<Escape>")
        print("Continue connection mode cancelled")  # Debug
    ...
    def run_python_code(self, filename):
        ...
        try:
            ...
            messagebox.showerror("Run Error", f"Could not run Python code: {e}")
    ...
    def import_package(self):
        ...
        try:
            ...
            added_count, actual_category = self.block_loader.load_package_file(filename, category_name)

            if added_count > 0:
                # Save custom blocks
                self.block_loader.save_custom_blocks()
            ...
            if isinstance(package_data, dict):
                ...
                for category, blocks in package_data.items():
                    if isinstance(blocks, list):
                        ...
                        if use_category not in self.block_loader.available_blocks:
                            self.block_loader.available_blocks[use_category] = []

                        # Check for duplicates
                        existing_block_texts = [b["text"] for b in self.block_loader.available_blocks[use_category]]
                        for block in blocks:
                            if block.get("text") not in existing_block_texts:
                                self.block_loader.available_blocks[use_category].append(block)
                                added_count += 1
            elif isinstance(package_data, list):
                # If it's a list, use the provided category name
                if category_name not in self.block_loader.available_blocks:
                    self.block_loader.available_blocks[category_name] = []

                # Check for duplicates
                existing_block_texts = [b["text"] for b in self.block_loader.available_blocks[category_name]]
                for block in package_data:
                    if block.get("text") not in existing_block_texts:
                        self.block_loader.available_blocks[category_name].append(block)
                        added_count += 1
            ...
            if added_count > 0:
                ...
                messagebox.showinfo(
                    "Package Imported",
                    f"Successfully imported {added_count} block(s) from '{package_name}'.\n"
                ...
                if hasattr(self, 'category_var'):
                    ...
                    if hasattr(self, 'blocks_tree'):
                        # Find the category combobox in the UI
                        for child in self.root.winfo_children():
            ...
            def add_selected():
                ...
                # Add selected functions as blocks
                for idx in selected_indices:
                    ...
                    block_data = {
                        "type": "function",
                        "text": func["name"],
                        "content": f"{package_name}.{func['name']}({func['params']})",
                        "template": f"{package_name}.{func['name']}({func['params']})",
                        "description": f"Function from {package_name} package"
                    }
                    
                    # Add to available blocks
                    if "Imported" not in self.block_loader.available_blocks:
                        self.block_loader.available_blocks["Imported"] = []
                    
                    self.block_loader.available_blocks["Imported"].append(block_data)
                ...
                messagebox.showinfo("Success", 
                                  f"Added {len(selected_indices)} function(s) from {package_name}")
                func_window.destroy()
            
            tk.Button(button_frame, text="Add Selected", command=add_selected).pack(side=tk.RIGHT)
            tk.Button(button_frame, text="Cancel", command=func_window.destroy).pack(side=tk.RIGHT, padx=5)
            
        except ImportError:
            messagebox.showerror("Import Error", 
                               f"Could not import package: {package_name}")
        except Exception as e:
            messagebox.showerror("Error", f"Error importing package: {e}")
