...
class BlockLoader:
    def __init__(self):
        ...
        # REMOVED: self.load_packages()  # Don't load packages on startup
    ...
    def load_custom_blocks(self):
        ...
        if os.path.exists(self.custom_blocks_file):
            try:
                ...
                for category, blocks in custom_blocks.items():
                    if category in self.available_blocks:
                        self.available_blocks[category].extend(blocks)
                    else:
                        self.available_blocks[category] = blocks
            except:
                pass

    def save_custom_blocks(self):
        """Save custom blocks to file"""
        try:
            with open(self.custom_blocks_file, 'w') as f:
                json.dump(self.available_blocks, f, indent=2)
        except:
            pass